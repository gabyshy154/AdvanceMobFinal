...
const HomeScreen = () => {
  ...
  // ===== Main Pokédex state =====
  const [pokemon, setPokemon] = useState<Pokemon[]>([]);     // Full list
  const [filtered, setFiltered] = useState<Pokemon[]>([]);   // Filtered list
  ...
  const fetchPokemon = async () => {
    ...
    try {
      ...
      setError('Failed to load Pokémon. Check your internet connection and try again.');
    } finally {
      setLoading(false);
    }
  };

  // ===== Fetch detailed Pokémon info for modal =====
  const fetchPokemonDetails = async (id: number) => {
    ...
    try {
      ...
      setSelectedPokemon(detailedPokemon); // Open modal with this data
    } catch (e) {
  ...
  const renderError = () => (
    <View style={styles.errorContainer}>
      ...
      <TouchableOpacity onPress={fetchPokemon}>
