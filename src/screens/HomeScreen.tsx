...
const HomeScreen = () => {
  ...
  const fetchPokemonDetails = async (id: number) => {
    ...
    try {
      ...
      const detailedPokemon: Pokemon = {
        ...
        description: flavorText?.flavor_text.replace(/\f/g, ' ') || 'No description available.',
      };
  ...
  const renderItem = ({ item }: { item: Pokemon }) => {
    ...
    return (
      ...
      >
        <View style={[styles.cardBackground, { backgroundColor: bgColor + '20' }]}>
  ...
  const renderStatBar = (statName: string, value: number) => {
    ...
    const statColor = value > 100 ? '#4CAF50' : value > 50 ? '#FFC107' : '#FF5722';

    return (
      <View style={styles.statRow} key={statName}>
        <Text style={styles.statName}>{statName.toUpperCase()}</Text>
        <View style={styles.statBarContainer}>
          <View style={[styles.statBarFill, { width: `${percentage}%`, backgroundColor: statColor }]} />
        </View>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBox}>
            <Text style={styles.statNumber}>
              {Math.round((filtered.length / 151) * 100)}%
            </Text>
            ...
            {detailLoading ? (
              ...
              <>
                ...
                <View style={[styles.modalHeader, {
                  backgroundColor: TYPE_COLORS[selectedPokemon.types[0]] || '#A8A878'
                }]}>
                  <TouchableOpacity
                  ...
                  <View style={styles.section}>
                    ...
                    <View style={styles.physicalStats}>
                      <View style={styles.physicalStatItem}>
                        ...
                        <Text style={styles.physicalStatValue}>{selectedPokemon.height} m</Text>
                      </View>
                      <View style={styles.physicalStatDivider} />
                      <View style={styles.physicalStatItem}>
                        <Text style={styles.physicalStatLabel}>Weight</Text>
                        <Text style={styles.physicalStatValue}>{selectedPokemon.weight} kg</Text>
                      </View>
                    </View>
                  </View>
...
const styles = StyleSheet.create({
  ...
  avatarButton: {
    width: 48, height: 48, borderRadius: 24, backgroundColor: '#DC0A2D',
    justifyContent: 'center', alignItems: 'center',
    shadowColor: '#DC0A2D', shadowOffset: { width: 0, height: 4 },
  ...
  statNumber: { fontSize: 20, fontWeight: 'bold', color: '#DC0A2D', marginBottom: 2 },
  ...
  imageWrapper: { alignItems: 'center', justifyContent: 'center', height: 100, position: 'relative' },
  imageBg: { position: 'absolute', width: 80, height: 80, borderRadius: 40 },
  pokemonImage: { width: 90, height: 90, zIndex: 1 },
  cardFooter: { paddingHorizontal: 12, paddingVertical: 12, backgroundColor: '#fff' },
  pokemonName: { fontSize: 15, fontWeight: 'bold', color: '#1a1a1a', marginBottom: 6 },
  typesContainer: { flexDirection: 'row', gap: 6 },
  typeBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
  typeText: { color: '#fff', fontSize: 10, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
